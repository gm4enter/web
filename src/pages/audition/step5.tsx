    ...
    const handleClickSave = async () => {
        ...
        try {
            ...
            const dataStep5: DataApplyArtist = {
                name: data.dataStep2?.name.trim() || '',
                gender: (data.dataStep2?.gender === '1' ? '남' : '여'),
                birthday: data.dataStep2?.dob || '',
                national: data.dataStep2?.countries.join(', ') || '',
                phone_number: data.dataStep2?.phoneNumber.trim() || '',
                email: data.dataStep2?.email.trim() || '',
                support_field: data.dataStep3?.supportType.join(', ') || '',
                height: parseFloat(data.dataStep3?.height.trim() || '0'),
                weight: parseFloat(data.dataStep3?.weight.trim() || '0'),
                postal_code: data.dataStep3?.postalCode || '',
                address: data.dataStep3?.address.trim() || '',
                career: data.dataStep3?.job.trim() || '',
                blood_group: data.dataStep3?.bloodGroup || '',
                languages: String(data.dataStep3?.language) || '',
                hobby: data.dataStep3?.hobby.trim() || '',
